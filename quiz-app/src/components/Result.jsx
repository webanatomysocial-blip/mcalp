...
function Result() {
  ...
  let riskDescription = 'Strengthen controls and evidence to close gaps.';
  ...
  if (yesCount >= 12) {
    ...
    riskDescription = 'Strengthen controls and evidence to close gaps.';
    ctaText = 'Get Gap-Closure Plan';
    badgeClass = 'badge-low';
  } else if (yesCount >= 9) {
    riskMessage = "You're Making Progress.";
    riskDescription = 'Your SAP setup is partially compliant. Configure critical tables and automated controls.';
      ...
      <div className="result-content">
        ...
        <div className="riskometer-section">
          <div className="riskometer-wrapper">
            <svg className="riskometer-svg" viewBox="0 0 200 110" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <linearGradient id="riskGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" style={{ stopColor: '#00d084', stopOpacity: 1 }} />
            ...
            <div className="gauge-labels-bottom">
              <span className={`gauge-label ${needleClass === 'risk-low' ? 'active' : ''}`}>LOW</span>
              <span className={`gauge-label ${needleClass === 'risk-medium' ? 'active' : ''}`}>MODERATE</span>
              <span className={`gauge-label ${needleClass === 'risk-high' ? 'active' : ''}`}>HIGH</span>
            </div>
          </div>
        </div>
        ...
        <div className="result-actions-grid">
          <a href={ctaLink1} className="cta-button-result primary" target="_blank" rel="noopener noreferrer">
            {ctaText}
          </a>
          <a href={ctaLink} className="cta-button-result secondary" target="_blank" rel="noopener noreferrer">
            Book 15-min Review
          </a>
        </div>
        ...
        <div className="additional-actions">
          ...
          <button className="action-button" onClick={() => navigate('/view-answers')}>
            View My Answers
          </button>
        </div>
      </div>
